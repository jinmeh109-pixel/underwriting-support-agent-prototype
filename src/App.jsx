...
function App() {
  ...
  useEffect(() => {
    ...
    setAgentAnswer('Select a question to see a deterministic plain-language response.');
    setHumanRationale('');
    setReviewMessage('');
  }, [selectedCaseId]);
...
function Dashboard({ setPage, selectedCase, assessment, auditRecords }) {
  const analyzed = sampleNewCases.length;
  const high = sampleNewCases.map(assessRisk).filter((risk) => ['High', 'Elevated'].includes(risk.level)).length;
  const reviews = sampleNewCases.map(assessRisk).filter((risk) => risk.humanReviewRequired).length;
  const stats = [
    ['Historical records loaded', historicalUploadSummary.recordsLoaded.toLocaleString()], ['Sample new cases available', sampleNewCases.length], ['Cases analyzed', analyzed], ['High-risk / elevated-risk cases', high], ['Human reviews required', reviews], ['What-if simulations run', auditRecords.filter((r) => r.tags?.includes('Scenario tested')).length], ['Audit records created', auditRecords.length], ['Data quality status', historicalUploadSummary.dataQualityScore],
  ];
  return <Page title="Executive workflow dashboard" subtitle="A presentation-ready overview of a frontend-only underwriting support workflow.">
    ...
    <div className="mt-6 grid grid-cols-3 gap-6"><Card className="col-span-2"><SectionTitle title="Top risk themes" subtitle="Themes used in transparent, deterministic sample-data reasoning." /><div className="flex flex-wrap gap-3">{['Vehicle age', 'Collision severity', 'Injury severity', 'Road surface', 'Weather condition', 'Time of day', 'Collision configuration'].map((theme) => <Chip key={theme}>{theme}</Chip>)}</div></Card><Card><SectionTitle title="Selected sample case" subtitle={selectedCase.id} /><p className="text-sm text-slate-600">{selectedCase.description}</p><p className="mt-4 rounded-xl bg-slate-100 p-3 text-sm font-medium text-slate-700">Current result: {assessment.level}</p></Card></div>
